...
router.post('/generate', (req, res) => {
    ...
    // access db based on username, update doc with pub/priv key pair, catch error
    User.findOne({ username: username })
        .then(usr => {
            const _id = usr._id;
            User.findOneAndUpdate({ _id: _id }, { public_key: publicKey, private_key: privateKey }, { upsert: true }, (err, doc) => {
                res.render('users', { publickey_store_success: 'RSA Key Pair Generated & Stored.', title: 'BitPay Developer Assessment', signature: null, username: username, missing_key: 'hidden', publickey: publicKey });
            });
        })
        .catch(err => {
...
// route digital signing
router.post('/sign', (req, res) => {
    var private_key, private_key_send, show = false;
    const username = req.body.username,
        message = req.body.message;
    // find user in DB, grab private key
    User.findOne({ username: username })
        .then(usr => {
            ...
            // if private key in db, sign and render signature
            if (private_key) {
                ...
                // if private key in req header, format, sign and render signature
                ...
                // if no private key, return error message with instructions to user
            } else {
                show = true;
...
// route message+signature verification, grab headers
router.post('/verify', (req, res) => {
    const signature = req.body.signature,
        username = req.body.username,
        message = req.body.message;
    // find user by username, grab public key, verify message using public key+signature, return success message
    User.findOne({ username: username })
        .then(usr => {
